...
\textbf{Public repository and release artifacts.}
The canonical public source for this artifact package is \url{https://github.com/tangodelta217/nema-connectome-artix7}, and reproducibility bundles/checksums are published through the corresponding releases page \url{https://github.com/tangodelta217/nema-connectome-artix7/releases}.
...
\textbf{Independent checker (anti-circularity).}
To reduce self-certification risk, the artifact includes an independent verifier (\texttt{tools/independent\_check.py}) that does not reuse the main gate logic. It re-validates digest consistency from trace/state files, checks a minimal \texttt{bench\_report.json} contract, and cross-checks selected HLS/Vivado metrics by direct regex extraction from report files.
\begin{verbatim}
python tools/independent_check.py --paperA
\end{verbatim}
PASS criterion: exit code \texttt{0} with no independent consistency errors.
