...
def _collect_verify_metadata(repo_root: Path) -> dict[str, dict[str, Any]]:
    ...
    for evidence_name in ("audit_software.json", "audit_hardware.json"):
        ...
        for check in checks.values():
            ...
            if not isinstance(bench_report, str):
                continue
            key = str(Path(bench_report).resolve())
...
def main() -> int:
    ...
    verify_meta = _collect_verify_metadata(repo_root)
    report_paths = _collect_candidate_report_paths(repo_root)

    selected: dict[tuple[str, str], tuple[datetime, BitExactRow]] = {}
    ...
    for report_path in report_paths:
        ...
        verify_info = verify_meta.get(str(report_path.resolve()), {})
