...
\item A normative fixed-point and tick-level semantics (\texttt{nema.tick.v0.1}) defined as a discrete-time leaky integrator with tanh-LUT activation, current-based chemical synapses, symmetric ohmic gap junctions, delayed snapshot reads via \texttt{delayTicks}, Euler update, exact RNE+SAT behavior, and bit-exactness defined from \texttt{V\_raw}-only digest traces.
...
In \texttt{nema.tick.v0.1}, the persistent per-node state is voltage $V$ in signed Q8.8 (int16 raw). Per tick, the simulator derives activation $A$ from a tanh LUT (Q8.8) and uses temporary current accumulators $I_{chem}$ and $I_{gap}$ in signed Q12.8 (20-bit raw domain). Delay handling uses \texttt{delayTicks} by reading prior voltage/activation snapshots from ring buffers; there are no digest-visible delay-bucket structures. Each tick follows a strict snapshot rule: (1) snapshot all voltages; (2) compute $A_i=\mathrm{tanhLUT}(V^{snapshot}_i)$; (3) accumulate chemical current over directed edges with $msg=\mathrm{quantize}_{Q12.8}(coeff\cdot A_{pre})$ and saturating adds; (4) accumulate gap current once per undirected pair $(a,b)$ with $msg=\mathrm{quantize}_{Q12.8}(g\cdot(V_b-V_a))$ and saturating $\pm msg$ updates; (5) compute Euler update from snapshot currents, $V_{next}=\mathrm{saturate}_{Q8.8}(V^{snapshot}+\mathrm{quantize}_{Q8.8}((dt/\tau_m)\cdot(I_{chem}+I_{gap})))$.

\subsection{Bit-exactness}
End-to-end equality is defined operationally rather than approximately. At every observed tick, only $V$ raw values are serialized in node-index order as signed int16 little-endian and hashed with SHA-256. Two implementations are bit-exact on a benchmark iff every observed tick yields the same digest.
...
The intended hardware organization is a deterministic CSR-based, time-multiplexed engine. Chemical edges are stored in CSR by post-neuron, gap junctions are stored as canonical endpoint-pair tables, and delayed reads are supported via BRAM-friendly ring buffers for prior voltage/activation snapshots. The hardware schedule mirrors the normative tick rule exactly: snapshot voltages, LUT activations, chemical accumulation, gap-pair accumulation, then Euler voltage update.
...
\centering
\small
\resizebox{\linewidth}{!}{\input{../review_pack/tables/artix7_hls_digest_summary_strict_v2.tex}}
...
\centering
\small
\resizebox{\linewidth}{!}{\input{../review_pack/tables/artix7_qor_v6.tex}}
...
\centering
\small
\resizebox{\linewidth}{!}{\input{../review_pack/tables/artix7_power_v7_funcsaif.tex}}
...
\centering
\small
\resizebox{\linewidth}{!}{\input{../review_pack/tables/artix7_metrics_final.tex}}
